...
Uses as full observation to generate a series of 10 minute dynamic spectra.
Supports:
357 solar bst observations.


This code will go through old data in archive and save 10 min dynamic spectra for
display in Monitor Archive. 
...
if __name__ == "__main__":
    ...
    # find the indices of the corresponding frequencies for lightcurves
    freqs_idx = list()
    ...
    intervals = int(len(times_X) / 600)
    if len(times_X) % 600 != 0:
        print(f"{intervals+1} intervals in this observation. Calculating estimated finish time...")
    else:
        print(f"{intervals} intervals in this observation. Calculating estimated finish time...")

    t1 = times_X[0]
    # Default output directory is a ./monitor/YYYY.MM.DD
    # otherwise manually add output directory.
    if len(sys.argv) <= 2:
        png_dir = os.getcwd() + '/monitor/' + str(t1.datetime.year) + '.' + str(t1.datetime.month).zfill(2) + '.' + str(t1.datetime.day).zfill(2)
    else:
        png_dir = sys.argv[2]
    ...
    # # # # # # # # # # # # # # # #
    # Going through observation   #
    # # # # # # # # # # # # # # # #
    for i in range(0, intervals):
        eta0 = datetime.datetime.now()
        t0 = t1
        t1 = times_X[(i+1)*600]

        # this is to make sure the file has 10 mins of data
        if len(times_X) > 599:
            savelightcurve(light_X[:, (i)*600:(i+1)*600], times_X[(i)*600:(i+1)*600], freqs_idx, file_357 + 'X.dat', png_dir, 'X')
            savelightcurve(light_Y[:, (i)*600:(i+1)*600], times_Y[(i)*600:(i+1)*600], freqs_idy, file_357 + 'Y.dat', png_dir, 'Y')

            savespectro(file_357 + 'X.dat', t0, t1, png_dir, 'X')
            savespectro(file_357 + 'Y.dat', t0, t1, png_dir, 'Y')

        eta1 = datetime.datetime.now()
        estimatedtimeleft = (intervals-i)*(eta1-eta0)
        print(f'{i}/{intervals}    ||   Estimated time left: {estimatedtimeleft}')
    ...
    if len(times_X)%600 != 0:
        ...
        # this is to make sure the file has 10 mins of data
        if len(times_X) > 599:
            savelightcurve(light_X[:, (i)*600:-1], times_X[(i)*600:-1], freqs_idx, file_357 + 'X.dat', png_dir, 'X')
            savelightcurve(light_Y[:, (i)*600:-1], times_Y[(i)*600:-1], freqs_idy, file_357 + 'Y.dat', png_dir, 'Y')

            savespectro(file_357 + 'X.dat', t0, t1, png_dir, 'X')
            savespectro(file_357 + 'Y.dat', t0, t1, png_dir, 'Y')

        print(f"{i} plots generated per polarization.")
